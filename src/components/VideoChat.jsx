...
export default function VideoChat({ gameId, currentUser, opponentId, socket, externalSignals, gameStatus }) {
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { endCall(); } }}>
            <DialogContent className="max-w-3xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2"><Video className="w-4 h-4" /> {t('game.video_chat')}</DialogTitle>
                </DialogHeader>

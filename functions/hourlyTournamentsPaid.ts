...
Deno.serve(async (req) => {
  try {
    ...
    // Paid category: choose a simple flat entry fee (10 D$) for hourly cadence
    const entryFee = 10;
    const created = [];
    const commonDesc = `Tournoi payant DamCash (${entryFee} D$) • Départ ${nextLocal.toLocaleString('fr-FR', { timeZone: 'Australia/Brisbane' })}`;

    // Chess (paid)
    created.push(await ensureTournament(base44, {
      name: `DamCash ${timeControl} • Échecs (Payant ${entryFee} D$)`,
      description: commonDesc,
      game_type: 'chess',
      time_control: timeControl,
      start_date_iso: startISO,
      entry_fee: entryFee,
      categoryLabel: 'Payant'
    }));

    // Checkers (paid)
    created.push(await ensureTournament(base44, {
      name: `DamCash ${timeControl} • Dames (Payant ${entryFee} D$)`,
      description: commonDesc,
      game_type: 'checkers',
      time_control: timeControl,
      start_date_iso: startISO,
